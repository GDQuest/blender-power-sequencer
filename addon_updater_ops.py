...
def update_settings_ui(self, context):
    ...
    if updater.error != None:
        ...
        split.scale_y = 2
        split.operator(
            POWER_SEQUENCER_OT_addon_updater_check_now.bl_idname,
            text="",
            icon="FILE_REFRESH"
        )
        ...
        split.scale_y = 2
        split.operator(
            POWER_SEQUENCER_OT_addon_updater_end_background.bl_idname,
            text="",
            icon="X"
        )
...
def skip_tag_function(self, tag):
    ...
    #
    # # Filter out e.g. if 'alpha' is in name of release
...
def register(bl_info):
    ...
    updater.user = "GDquest"

    # choose your own repository, must match git name
    updater.repo = "Blender-power-sequencer"

    #updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set
    updater.website = "https://github.com/GDquest/Blender-power-sequencer/releases"
