...
class POWER_SEQUENCER_MT_main(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator("power_sequencer.synchronize_titles", text='Synchronize Titles', icon='SORTALPHA')
        layout.operator("power_sequencer.set_timeline_range", text='Set Timeline Range', icon='ARROW_LEFTRIGHT')
...
class POWER_SEQUENCER_MT_strips(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("power_sequencer.concatenate_strips", text='Concatenate Strips', icon='CONSTRAINT')
        layout.operator("power_sequencer.swap_strips", text='Swap Strips', icon='ARROW_LEFTRIGHT')
        layout.operator("power_sequencer.toggle_selected_mute", text='Toggle Mute', icon='MUTE_IPO_ON')
        layout.operator("power_sequencer.channel_offset", text='Channel Offset', icon='AUTOMERGE_ON')
        ...
        layout.operator("power_sequencer.make_still_image", text='Make Still Image', icon='FILE_IMAGE')
...
class POWER_SEQUENCER_MT_deselect(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator("power_sequencer.deselect_all_strips_left_or_right", text='Deselect Left/Right')
...
class POWER_SEQUENCER_MT_snap(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator("power_sequencer.trim_left_or_right_handles", text='Trim Left/Right Handles')
...
class POWER_SEQUENCER_MT_edit(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("power_sequencer.copy_selected_sequences", text='Copy Selected Sequences', icon='COPYDOWN')
...
class POWER_SEQUENCER_MT_markers(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("power_sequencer.marker_go_to_next", text='Go to Next Marker', icon='TRIA_RIGHT_BAR')
        layout.operator("power_sequencer.copy_markers_as_timecodes", text='Copy Marker as Timecodes')
        layout.operator("power_sequencer.marker_snap_to_cursor", text='Snap Marker to Cursor')
        layout.operator("power_sequencer.set_preview_between_markers", text='Set Preview Between Markers')
        layout.operator("power_sequencer.markers_snap_matching_strips", text='Snap Matching Strips', icon='NLA')
...
class POWER_SEQUENCER_MT_file(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator("power_sequencer.open_project_directory", text='Open Project Directory', icon='FILE_FOLDER')
        layout.operator("power_sequencer.save_direct", text='Direct Save', icon='FILE_TICK')
        layout.operator("power_sequencer.import_local_footage", text='Import Local Footage', icon='FILE_MOVIE')
...
class POWER_SEQUENCER_MT_trim(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator("power_sequencer.trim_three_point_edit", text='Trim Three Point Edit', icon='TRIA_DOWN')
        layout.operator("power_sequencer.trim_to_surrounding_cuts", text='Trim to Surrounding Cuts', icon='SNAP_PEEL_OBJECT')
...
class POWER_SEQUENCER_MT_audio(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("power_sequencer.toggle_waveforms", text='Toggle Waveforms', icon='RNDCURVE')
