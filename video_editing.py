...
class ConcatenateStrips(bpy.types.Operator):
    ...
    concatenate_whole_channel = BoolProperty(
        name="Concatenate all strips in channel",
        description="If only one strip selected, concatenate the entire channel", default=False)
    ...
    def execute(self, context):
        ...
        if len(sequences) == 1:
            ...
            in_channel = [s for s in find_next_sequences(first_strip) if s.channel == first_strip.channel]

            for s in in_channel:
                sequences.append(s)

        sequences = filter_sequences_by_type(sequences,
                                             SequenceTypes.VIDEO,
                                             SequenceTypes.IMAGE,
                                             SequenceTypes.SOUND)
        if len(sequences) <= 1:
