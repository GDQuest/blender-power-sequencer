        ...
        if self._tags == None:
            ...
            if self._verbose:
                ...
                print("{} branch found, no releases".format(branch), self._tags[0])
                 ...
                 ) - len(self._include_branch_list) == 0 and self._prefiltered_tag_count > 0:
            self._tag_latest = None
            ...
            for file in files:
                ...
                if os.path.isfile(destFile):
                    ...
                    if replaced:
                        ...
                        if self._verbose:
                            print(
                                "Pattern not matched to " + os.path.basename(destFile) +
                                ", not overwritten"
                            )
                else:
...
class BitbucketEngine(object):
    ...
    def parse_tags(self, response, updater):
        ...
        return [{
            "name": tag["name"],
            "zipball_url": self.get_zip_url(tag["name"],
                                            updater)
...
class GitlabEngine(object):
    ...
    def parse_tags(self, response, updater):
        ...
        return [{
            "name": tag["name"],
            "zipball_url": self.get_zip_url(tag["commit"]["id"],
                                            updater)
        } for tag in response]
